...
- **Done:**
  ...
  - Added no-arg constructor to `LibraryService`
  - Added `getBooks()`, `getRecords()`, `setBooks()`, `setRecords()`, `setNextBookId()`, `setNextRecordId()` to `LibraryService`
  - Restored atomic temp-file save in `FileStorage` (write to `_temp.txt`, then `Files.move`)
  ...
  - Created `README.md` following `Template.md` structure
